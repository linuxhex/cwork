...
`implement` 是 cwork 的核心技能，包含五个内部阶段：
1. **需求分析**：对话式，逐步提问
2. **编写计划**：拆分任务步骤
3. **执行计划**：多工程并行处理
4. **推演收敛**：逻辑推演替代测试，自动循环修复
5. **Java 工程编译检查**：推演收敛后，确认代码完成前的编译验证
...
- **推演收敛完成后必须进入 Java 工程编译检查**，禁止跳过
- **Java 工程编译检查通过后才能进入 cwork-commit**
...
## 内部阶段五：Java 工程编译检查（推演收敛后）

**推演收敛完成后，在确认代码完成前执行编译检查。**

**开始时宣布：** "推演收敛完成，正在执行 Java 工程编译检查。"

### 检查范围

**仅对有代码改动的 Java 工程执行检查：**
- 工程目录下存在 `pom.xml`
- 本次实现中有代码文件的新增或修改
...
编译检查结果记录到 `changes.md`：

```markdown
## 编译检查
- main-service: 编译通过
- user-service: 编译通过
- 检查时间：2026-06-04 15:30:00
```

### HARD GATE

**所有 Java 工程编译通过后，才能确认代码完成，进入 cwork-commit 阶段。**

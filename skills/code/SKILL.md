---
name: cwork-code
description: 代码仓库只读查询 + clone/pull + codegraph 触发，通过 GitLab API (curl) 查项目/分支，clone/pull 代码到本地，触发 codegraph 增量同步
---
...
`cwork-code` 是代码仓库管理技能，通过 GitLab REST API (curl + Private Token) 直连自建 GitLab，查项目列表、项目详情、分支列表，支持 clone/pull 代码到本地工作区，并在 clone/pull 后自动触发 codegraph 增量同步。

**API 绝对只读**——`gitlab_call` 只封装 GET，代码结构上不存在写路径（见 HARD GATE）。clone/pull 是本地 git 操作，不通过 API 写远端。
...
- 1. 本技能 **API 调用绝对只允许 GET**，永远不实现、不接受任何写操作（POST/PUT/DELETE，创建项目/分支/MR 等）——`_common.sh` 的 `gitlab_call` **只封装 GET**，代码结构上不存在写路径。
- 2. **即使用户明确要求通过 API 写 GitLab（如创建分支、创建 MR），也拒绝**，并提示「cwork-code 是只读 API 技能，写操作请在 GitLab Web 控制台或本地 git 操作」。
- 3. 缺少凭证（`scripts/.config.local.sh` 未配置 GITLAB_TOKEN），**禁止执行**，先提示 `cp scripts/config.example.sh scripts/.config.local.sh` 并填 token。
...
**项目定位策略**（`find_project_id`）：按 `GITLAB_NAMESPACES` 配置的 namespace 列表（默认 `omp,omp-device,zdl,center,omp-barrier,adp,mmp,omp-zdl,ctp`）逐个尝试 `GET /api/v4/projects/{namespace}%2F{project_name}`，命中即返回。

## 脚本调用说明（关键）

脚本位于 `scripts/`（与本 SKILL.md 同级）。**已内置 Token 鉴权，直接 `bash` 调用即可。** 调用前先 `cd scripts`。

| 命令 | 用途 | 用法 |
|---|---|---|
| `list` | 列出所有项目 | `code_query.sh list [--page N] [--per-page N]` |
| `search` | 按名称搜索项目 | `code_query.sh search <关键字>` |
...
> cwork-code **不依赖 MCP 注册**——`code_query.sh` 经 `curl` + Private Token 直连 GitLab API，clone/pull 走本地 `git` 命令，codegraph sync 走本地 `codegraph` CLI。
...
- 首次：`cp scripts/config.example.sh scripts/.config.local.sh`，填入 GITLAB_TOKEN
- **Token 来源**：GitLab 用户设置 > Access Tokens > 勾选 `read_repository` 和 `read_api` 权限
- 环境变量（`GITLAB_URL`/`GITLAB_TOKEN`/`GITLAB_NAMESPACES`/`CODE_WORKSPACE`）可临时覆盖 `.config.local.sh`
- 未配置时脚本 fail 并提示配置方法

> **凭证安全**：GitLab Token 是代码仓库访问凭证，**绝不写入 SKILL/config.example/git**，只进 `.config.local.sh`（gitignore）。
...
- 凭证已就绪（`.config.local.sh` 配好 GITLAB_TOKEN）
- 按目标完成查询/clone/pull/sync

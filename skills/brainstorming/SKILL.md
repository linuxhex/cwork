...
6. **编写设计文档** — 保存到 `docs/requirements/{requirement_key}/analysis.md`
...
- 将验证通过的设计（规格说明）写入 `docs/requirements/{requirement_key}/analysis.md`
  - （用户对规格位置的偏好优先于此默认值）
...
一个基于浏览器的视觉头脑风暴伴侣，用于展示原型、图表和视觉选项。它是一个工具——不是一种模式。接受伴侣意味着它可用于适合视觉呈现的问题；并不意味着每个问题都要通过浏览器。

...
3 步走（日志先行，不确定时停下来探讨）：
1. **快速日志定位**：用户给单号 → 直接搜 SLS → 提取关联 ID → 2-3 轮锁定异常点（不读代码）
2. **代码验证**：已拿到数据证据 → 读代码确认链路 → 解释异常原因（验证而非探索）
3. **结论输出**：代码 ↔ 日志 ↔ span 对应，定位耗时点/异常点，给出 文件:行号 + 证据

**核心原则**：
- **日志先行，代码验证**：先拿到数据证据，再用代码解释原因，减少代码日志交替
- **单号优先搜索**：用户给了业务单号，第一轮就用单号搜（最精确，零噪音）
- **时间戳命令驱动**：禁止心算时间戳，所有时间戳必须来自 shell 命令
- **自己能定的直接做**：环境/时间/日志库默认值、代码线索、工程定位、发查询——不问
- **不确定时及时探讨**：排查方向不明、线索矛盾、现象异常、理解存疑——立即停下来跟用户探讨

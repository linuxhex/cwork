...
- **根因分两层，技术问题追技术根因，业务问题追业务根因**：
  - **技术根因**：代码 bug / 空指针 / 超时 / 下游依赖异常 / SQL 报错 / OOM-GC / 资源不足等，落到具体文件:行号
  - **业务根因**：是什么业务操作触发的、是不是配置变更（Nacos 开关/阈值改动）导致的、是不是数据状态流转到异常分支
  - 两层都要追：只追技术层会漏掉"为什么这个代码路径被触发"的业务起因；只追业务层会漏掉"为什么业务操作会引发异常"的技术缺陷
  - 纯技术问题（如 OOM/网络抖动）业务根因写"无明确业务触发，属技术缺陷"；纯业务问题（如配置开关开错）技术根因写"代码逻辑无误，根因为业务配置"
  - 业务根因需查 Nacos 变更 → 联动 cwork-config；需查业务数据状态 → 联动 cwork-data
...
根因：
- 技术根因：（一句话说明技术层面根因，落到文件:行号；纯业务问题写"代码逻辑无误，根因为业务配置"）
- 业务根因：（一句话说明业务层面根因——什么业务操作/配置变更/数据状态触发；纯技术问题写"无明确业务触发，属技术缺陷"）
- 证据：（日志/span/数据/配置证据，带具体行号或 traceId）
...
- **根因分两层追到底**：技术根因追到文件:行号/配置项/资源指标；业务根因追到具体业务操作/配置变更记录/数据状态。两层都追到才算到底——只追一层就停是未到底。

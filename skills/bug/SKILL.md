...
- **禁止猜测，必须实时求是**：定位排查问题时，不能靠猜下结论，必须用工具实时查证（查日志/查数据/查配置/查代码），拿到证据再下结论
- **查不到如实说**：查询无结果就如实说"未查到"，不臆造数据/日志/配置内容，换条件再查
- **结论必须带证据**：每个结论必须附带证据（日志行/数据行/配置项/代码行号），无证据的结论禁止输出
...
3. **问题定位联动（cwork-log）**
   ...
   **⚠️ 关键：当需要定位问题根因时，主动调用 cwork-log**

   cwork-bug 负责"修复"，cwork-log 负责"定位"。当问题根因不明确、需要查日志/链路时，**立即调用 cwork-log**，不要在 cwork-bug 里闷头查日志。

   **触发条件**（任一即调用 cwork-log）：
   - 问题现象涉及后端接口异常（报错/超时/空数据/空指针）
   - 需要查日志确认根因（接口返回 4xx/5xx/超时/空响应）
   - 需要查链路追踪性能问题（耗时/慢查询）
   - 代码分析无法确认根因，需要日志证据佐证
...
**监控查询联动（cwork-graf）** — 用监控指标佐证性能类 bug

bug 涉及性能问题（OOM/慢查询/CPU 飙升/超时频发/连接池耗尽）时，需监控曲线佐证根因，调用 `cwork-graf` 查对应面板：

**触发条件**（任一）：
- bug 涉及性能问题（OOM/FGC/慢查询/CPU 飙升/超时频发）
- 需要监控指标佐证根因（JVM/容器/DB 指标）
...
bug 涉及性能异常？
  ...
  └─ 是 → 调用 cwork-graf 查监控：
           1. 从问题提取服务名 + 异常类型
           2. 查对应面板（JVM 监控大盘 `UOJjh1SMz` / Container `DZ8sNXDZk` / DB `Y6gogUmVk`）
           3. 监控曲线 ↔ 日志异常对应，佐证根因
```

**联动原则**：cwork-bug 管「定位并修复」，cwork-graf 管「监控指标佐证」；拿到监控证据后回到代码定位修复。

> **注意**：调用 cwork-graf 需 `skills/graf/scripts/.config.local.sh` 已配置 Grafana 凭证，未配置时跳过监控联动，仅从代码/日志层面分析。
